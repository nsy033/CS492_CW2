...
parser.add_argument('--fake_first', dest='fake_first',   action='store_true', help='train with fake img first')

args = parser.parse_args()
perc_real = args.real
perc_fake = args.fake
num_train = args.size
fake_first = args.fake_first
...
x_train_fake = gen_imgs[:num_fake]
y_train_fake = sampled_labels[:num_fake]
x_train_real = X_train[:num_real]
y_train_real = y_train[:num_real]
...
clf.compile(optimizer='adam',
              ...
              metrics=['accuracy'])
x = 'fake_first'
num_fake_epoch=int(10*perc_fake)
num_real_epoch=int(10*perc_real)
# num_fake_epoch=5
# num_real_epoch=5
if fake_first:
    if num_fake!=0:
        clf.fit(x=x_train_fake,y=y_train_fake, epochs=num_fake_epoch)
    if num_real!=0:
        clf.fit(x=x_train_real,y=y_train_real, epochs=num_real_epoch)
else:
    if num_real!=0:
        clf.fit(x=x_train_real,y=y_train_real, epochs=num_fake_epoch)
    if num_fake!=0:
        clf.fit(x=x_train_fake,y=y_train_fake, epochs=num_real_epoch)
    x = 'real_first'
...
# new_path_save_model = 'save_weight_classifier/fake_real_serial/10epochs_' + str(100*perc_real) + '_' + str(100*perc_fake) + '_' + x +'.h5'
